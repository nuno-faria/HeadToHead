...
class FootballTree {
    ...
    public static DecisionTree CreateTree() {
		...
		DecisionTree ATTACK = new DecisionTree("ATTACK", (state, data) => data["_x"] = state["ballPosX"] + 0.25, (state, data) => Math.Abs(state["selfPosX"] - state["ballPosX"]) < 1 && Math.Abs(state["selfPosY"] - state["ballPosY"]) < 1 && Math.Abs(state["selfPosY"] - state["ballPosY"]) > 0.5, new List<DecisionTree> {JUMP, MOVE_TO_CUSTOM_POS}, new List<DecisionTree> {MOVE_TO_CUSTOM_POS});
